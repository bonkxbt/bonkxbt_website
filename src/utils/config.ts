const defaults = {
    ...
    chatbot_backend: process.env.NEXT_PUBLIC_CHATBOT_BACKEND ?? 'bonkxbt',
    ...
    vision_system_prompt: process.env.NEXT_PUBLIC_VISION_SYSTEM_PROMPT ?? `You are a friendly human named bonkxbt. Describe the image in detail. Let's start the conversation.`,
    ...
    name: process.env.NEXT_PUBLIC_NAME ?? 'bonkxbt',
    system_prompt: process.env.NEXT_PUBLIC_SYSTEM_PROMPT ?? `Hi, I'm bonkxbt! As a virtual financial advisor powered by advanced AI technology, my mission is to help every investor find their way in the fast-changing world of cryptocurrency.
